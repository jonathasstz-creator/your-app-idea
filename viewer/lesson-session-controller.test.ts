...
describe("LessonSessionController — endLesson stops everything", () => {
    ...
    it("frameLoop and timer are NOT stopped before endLesson is called", () => {
        const { controller, timer, frameLoop } = makeController();

        controller.startLesson();
        // No endLesson call
        expect(frameLoop.stop).not.toHaveBeenCalled();
        expect(timer.stop).not.toHaveBeenCalled();
    });
});

// ---------------------------------------------------------------------------
// Teste C — engine.onMidiInput que lança não propaga via handleMidi
// ---------------------------------------------------------------------------

describe("LessonSessionController — handleMidi isola erros do engine", () => {
    it("[RED→GREEN] engine.onMidiInput que lança não propaga via handleMidi", () => {
        ...
        // FALHA hoje: handleMidi propaga o throw (sem try/catch no controller)
        // VERDE após fix: try/catch em handleMidi isola o erro
        expect(() => controller.handleMidi(60, 100, true)).not.toThrow();

        // endLesson funciona independente do erro no engine
        controller.endLesson("COMPLETE");
        expect(controller.isEnded()).toBe(true);
        expect(timer.stop).toHaveBeenCalledTimes(1);
    });

    it("handleMidi retorna true (processou) mesmo quando engine lança", () => {
        const throwingEngine = {
            tick: vi.fn(),
            onMidiInput: vi.fn(() => { throw new Error("boom"); }),
            getViewState: vi.fn(() => ({ status: "RUNNING" })),
            forceEnd: vi.fn(),
        };
        const { controller } = makeController({ engine: throwingEngine });
        controller.startLesson();

        const result = controller.handleMidi(60, 100, true);
        expect(result).toBe(true); // ainda processou (engine foi chamado)
    });
});
